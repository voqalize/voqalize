...
from voqalize_demos.manifest import brain_factory, check_wiring, load_demos
...
def test_manifest_and_registry_agree():
    """Every demo declared in manifest.json has a bound brain, and vice versa."""
    demos = check_wiring()
    assert demos, "at least one demo must be declared"
    for demo in demos:
        assert brain_factory(demo.name) is not None


def test_travel_is_declared():
    names = {d.name for d in load_demos()}
    assert "travel" in names


def test_umbrella_app_builds():
    """The umbrella constructs (manifest wiring valid) and exposes the brain route."""
    app = create_app()
    paths = {getattr(r, "path", None) for r in app.routes}
    assert "/{name}/s/{session_id}" in paths
...
def test_unknown_demo_has_no_brain():
    """An unregistered name raises KeyError — the umbrella maps this to a 4000 close."""
    with pytest.raises(KeyError):
        brain_factory("does-not-exist")
