...
Voqalize dials `/interview_bot?session_id=…` once per session (the inbound path); the
...
def build(client: genai.Client) -> InterviewBotBrain:
    """Build a fresh brain for one session from the shared Gemini client."""
    return InterviewBotBrain(client=client)
