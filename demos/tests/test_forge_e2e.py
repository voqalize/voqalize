...
by the conformance ``VoqalizeDriver``, with only the *model* scripted. See
``tests/_harness.py`` for what every demo's e2e proves.

Forge inverts the usual ownership: the **browser** owns the workflow and Ada only
relays edits, grounded on the ``state_sync`` snapshot the studio pushes. So the
two things worth pinning are that a tool call reaches the studio with its
arguments intact (they are the store's op keys — a rename in either repo silently
drops the edit), and that the snapshot lands in the prompt without taking the
floor.
    ...
    return ScriptedGemini(
        {
            "Open the guest wifi workflow.": [
                reply_and_call("Opening it.", "open_workflow", id="guest-wifi"),
...
async def test_edits_reach_the_studio_with_their_arguments_intact() -> None:
    """Forge forwards tool arguments to the browser **verbatim**: the tool schema's
    arg names *are* the studio store's op keys.
...
async def test_the_workspace_snapshot_grounds_the_next_turn_silently() -> None:
    ...
    async with demo("forge", llm) as rig:
        ...
        check_turn(rig, second, inferences=1)
