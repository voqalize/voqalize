 ...
 * default, and the brain overrides it with one `session.configure(...)` in
 * `on_session_start` — the only place both the STT and the TTS leg move
 * together. A page that set one of them from the browser would be setting
 * exactly half of a pair, so this demo sends no `config` at all.
 *
...
export const demo: DemoConfig = {
  apiBase: "/api/v1",
  // Empty when unprovisioned — the control plane answers 401 and the bar shows it.
 ...
 * uninterpreted by everything between here and there. This demo's PHARMACY
 * CONTEXT (`buildBrainPayload`) rides here.
 *
